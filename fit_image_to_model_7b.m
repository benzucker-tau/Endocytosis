...
%% folder for results
%path1 = 'E:\Ben\neuro-endocytosis\aug 2019 images\lambda';   %%
path1 = 'E:\Ben\neuro-endocytosis\aug 2019 movies\movie 1 fit';   %%
...
%% first fit -> rough fit
%% go over test_number (10) simulations in every folder and rates the folderby goodness of fit -->>
%% not average rather best fit
...
leastsquares = -1 ;
for  folderindex = folder_to_scan'   %% run over foldrs of simulations results   ((replaced by -> '3 : length(subFolders)' ))
    cd(fullfile(results_folder,subFolders(folderindex).name));
    txt_files = dir('aperture*height*.txt');
    %% keep only data files in this folrder!
    for filenumber = 1:length(txt_files)    
        ...
        if isfile(filename)                          %% consition -> the file exists and not an outlier
            ...
            if (resnorm < leastsquares) || (leastsquares == -1)
                leastsquares    = resnorm ;
                bestfit         = filename ;
                bestfitfolder   = folders(folderindex).name ;
                bestscale       = scale ;
                bestheight      = height ;
...
plot(movie(str2num(video_number{1})).scale_meter*R,movie(str2num(video_number{1})).scale_meter*Z,'k','LineWidth',1.5);
hold on;
...
%{
%%%%% to go back from fit to paramteres
bestscale = movie(str2num(video_number{1})).snapshots(snap_number).natural_length_scale/movie(str2num(video_number{1})).scale_meter ;   %% calculating natural length scale
bestaperture = movie(str2num(video_number{1})).snapshots(snap_number).aperture/movie(str2num(video_number{1})).snapshots(snap_number).natural_length_scale ;
bestheight = movie(str2num(video_number{1})).snapshots(snap_number).height/movie(str2num(video_number{1})).snapshots(snap_number).natural_length_scale   ;
q = bestscale*load(movie(str2num(video_number{1})).snapshots(snap_number).simulation_location);
%}
